...
public:
    ...
    void init(LicenseDialog *q)
    {
        ...
        QObject::connect(m_ui.m_scrollArea->verticalScrollBar(), &QScrollBar::valueChanged, [this](auto value) {
            this->highlightAnchor(value);
        });
    }
                               ...
                               const QString &license)
{
    ...
    if (!first) {
        QFrame *line = new QFrame(m_d->m_w);
        line->setFrameStyle(QFrame::HLine | QFrame::Plain);
        m_d->m_l->addWidget(line);
    }
    ...
    Anchor *a = new Anchor(m_d->m_aw, title, label, m_d->m_ui.m_scrollArea);
    m_d->m_al->insertWidget(m_d->m_al->count() - 1, a);
    m_d->m_anchors.push_back(a);

    m_d->highlightAnchor(m_d->m_ui.m_scrollArea->verticalScrollBar()->value());
}

void LicenseDialog::showEvent(QShowEvent *e)
{
    m_d->highlightAnchor(m_d->m_ui.m_scrollArea->verticalScrollBar()->value());
